...
export default function RutaPage() {
  ...
  const [sugerencia, setSugerencia] = useState<Sucursal | null>(null);
  ...
  const generarRutaDia = async (forzar = false) => {
    if (!esDiaLaborable(fecha)) {
      alert('Este día no es laborable (domingo). Elige lunes a sábado.');
      return;
    }

    const minutosDisp = minutosProductivosDelDia(fecha);
    ...
    // Arrastrar pendientes parciales del día anterior al inicio
    const ayer = new Date(fecha + 'T12:00:00');
    ayer.setDate(ayer.getDate() - 1);
    const ayerStr = ayer.toISOString().split('T')[0];
    const { data: parcialesAyer } = await supabase
      .from('visitas_programadas')
      .select('*, sucursal:sucursales(*)')
      .eq('estado', 'parcial')
      .lte('fecha_programada', ayerStr);
    ...
    if (existentes && existentes.length > 0 && !forzar) {
      if (!confirm('Ya hay visitas pendientes para este día. ¿Regenerar solo lo pendiente?')) return;
      await supabase
        .from('visitas_programadas')
        .delete()
        .eq('fecha_programada', fecha)
    ...
    const visitasNuevas: Record<string, unknown>[] = [];
    let minutosUsados = 0;
    let ordenDia = 1;

    // Primero reprogramar parciales pendientes
    for (const p of parcialesAyer || []) {
      const t = p.sucursal?.tiempo_estimado_minutos || 60;
      if (minutosUsados + t > minutosDisp) break;
      visitasNuevas.push({
        ...
        estado: 'pendiente',
        es_emergencia: false,
        observaciones: `Continuación: faltaban ${p.aires_pendientes || 0} aires, ${p.mini_split_pendientes || 0} mini, ${p.equipos_grandes_pendientes || 0} grandes, ${p.bombas_pendientes || 0} bombas`,
      });
      minutosUsados += t;
      // marcar el parcial viejo como omitida/cerrado de arrastre
      await supabase.from('visitas_programadas').update({ estado: 'omitida' }).eq('id', p.id);
    }

    // Luego sucursales del ciclo
    let i = 0;
    const maxIter = sucursales.length * 2;
    while (minutosUsados < minutosDisp && i < maxIter) {
      const idx = (ordenActual + i) % sucursales.length;
      const s = sucursales[idx];
      const t = s.tiempo_estimado_minutos || 60;
      if (minutosUsados + t > minutosDisp) break;
      // evitar duplicar si ya está por parcial
      if (!visitasNuevas.some((v) => v.sucursal_id === s.id)) {
        visitasNuevas.push({
          sucursal_id: s.id,
          fecha_programada: fecha,
          orden_del_dia: ordenDia++,
    ...
    if (visitasNuevas.length === 0) {
      alert('No cabe ninguna visita en el horario de este día con los tiempos estimados actuales.');
      return;
    }
    ...
    if (ciclo) {
      await supabase
        .from('ciclo_mantenimiento')
        .update({ sucursal_actual_orden: (ordenActual + i) % sucursales.length })
        .eq('id', ciclo.id);
    }
    ...
    await registrarBitacora(
      ...
      'visitas_programadas',
      undefined,
      `${visitasNuevas.length} visitas el ${fecha} (~${Math.round(minutosUsados / 60)}h de ${minutosDisp / 60}h disponibles)`
    );
    load();
  };
  ...
  const guardarCierre = async () => {
    ...
    const sucursalNombre = selected.sucursal?.nombre || '';
    const sucursalId = selected.sucursal_id;
    const eraEmergencia = selected.es_emergencia;
    ...
    try {
      ...
      if (modoCierre === 'completa') {
        ...
        update.bombas_pendientes = bombasPend;
      }
      ...
      // Evidencias (no bloquear si falla storage)
      try {
      ...
      // Si era emergencia completada, cerrar problemas vinculados
      if (modoCierre === 'completa' && (eraEmergencia || sucursalId)) {
        try {
          await supabase
            .from('problemas')
            .update({
      ...
      try {
        await registrarBitacora(
          ...
          'visitas_programadas',
          visitaId,
          `${sucursalNombre}: ${trabajo || 'sin detalle'}`
        );
      } catch {
        // ignore
      }
      ...
      if (modoCierre === 'completa') {
        try {
          ...
          const { data: hechas } = await supabase
            .from('visitas_programadas')
            .select('tiempo_real_minutos, estado')
            .eq('fecha_programada', fecha)
            .in('estado', ['completada', 'parcial', 'en_progreso']);
          const usados =
            (hechas || []).reduce((a, h) => a + (h.tiempo_real_minutos || 60), 0) +
            Math.round(tiempoRealHoras * 60);
          const quedan = minutosDisp - usados;
          const ids = new Set([...(visitas.map((v) => v.sucursal_id)), sucursalId]);
          const candidata = todasSucursales.find(
            (s) => !ids.has(s.id) && (s.tiempo_estimado_minutos || 60) <= quedan
          );
          if (candidata && quedan >= 30) setSugerencia(candidata);
        } catch {
          // ignore
        }
      }
  ...
  const aceptarSugerencia = async () => {
    ...
    await supabase.from('visitas_programadas').insert({
      sucursal_id: sugerencia.id,
      fecha_programada: fecha,
      orden_del_dia: maxOrden + 1,
      estado: 'pendiente',
      es_emergencia: false,
    });
    await registrarBitacora('agregar_visita_extra', 'visitas_programadas', sugerencia.id, sugerencia.nombre);
    setSugerencia(null);
    load();
  };
      ...
      {sugerencia && (
        <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center gap-3">
          <div className="flex-1">
            <div className="font-semibold text-emerald-900">Queda tiempo en el día</div>
            <div className="text-sm text-emerald-800">
              ¿Agregar <strong>{sugerencia.nombre}</strong> ({formatHoras(sugerencia.tiempo_estimado_minutos)}) a la ruta de hoy?
            </div>
          </div>
          <div className="flex gap-2">
            <button onClick={aceptarSugerencia} className="px-3 py-2 bg-emerald-500 text-white rounded-xl text-sm font-medium">
              Sí, agregar
            </button>
            <button onClick={() => setSugerencia(null)} className="px-3 py-2 bg-white border border-emerald-200 text-emerald-800 rounded-xl text-sm">
              No
            </button>
          </div>
        </div>
