...
export default function RutaPage() {
  ...
  const guardarCierre = async () => {
    ...
    await registrarBitacora(
      ...
      'visitas_programadas',
      selected.id,
      `${selected.sucursal?.nombre}: ${trabajo || 'sin detalle'}`
    );

    // Si terminó completo y queda tiempo, sugerir otra sucursal
    if (modoCierre === 'completa') {
      const minutosDisp = minutosProductivosDelDia(fecha);
      ...
      const usados = (hechas || []).reduce((a, h) => a + (h.tiempo_real_minutos || 60), 0)
        + Math.round(tiempoRealHoras * 60);
      const quedan = minutosDisp - usados;
      const ids = new Set([...(visitas.map((v) => v.sucursal_id)), selected.sucursal_id]);
      const candidata = todasSucursales.find(
        (s) => !ids.has(s.id) && (s.tiempo_estimado_minutos || 60) <= quedan
      );
      if (candidata && quedan >= 30) {
