...
export default function ProblemasPage() {
  ...
  const eliminarProblema = async (id: string) => {
    if (!confirm('¿Eliminar este problema?')) return;
    await supabase.from('archivos_problema').delete().eq('problema_id', id);
  ...
  const marcarResuelto = async (id: string) => {
    ...
    await supabase
      ...
      .update({
        ...
        notas_resolucion: 'Marcado como resuelto',
      })
      .eq('id', id);
    load();
  };
            ...
            <div key={p.id} className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm">
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex flex-col gap-2 shrink-0">
                  {p.estado === 'abierto' && (
                    <button
                      onClick={() => marcarResuelto(p.id)}
                      className="text-sm px-3 py-1.5 bg-green-50 text-green-700 rounded-lg hover:bg-green-100"
                    >
                      Marcar resuelto
                    </button>
                  )}
                  {p.estado === 'abierto' && !p.convertido_a_emergencia && (
                    <button
                      ...
                      className="text-sm px-3 py-1.5 bg-red-50 text-red-700 rounded-lg hover:bg-red-100"
                    >
                      Crear emergencia
                    </button>
                  )}
                  {esAdmin && (
                    <button
                      ...
                      className="text-sm px-3 py-1.5 bg-slate-50 text-red-600 rounded-lg hover:bg-red-50"
                    >
                      Eliminar
                    </button>
                  )}
