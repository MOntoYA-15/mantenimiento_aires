...
export type EstadoVisita = 'pendiente' | 'en_progreso' | 'completada' | 'parcial' | 'omitida';
...
export interface ArchivoVisita {
  id: string;
  visita_id: string;
  url: string;
  tipo: 'imagen' | 'video';
...
export interface VisitaProgramada {
  ...
  aires_pendientes?: number;
  mini_split_pendientes?: number;
  equipos_grandes_pendientes?: number;
  bombas_pendientes?: number;
